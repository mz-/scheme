...
(define (list-partitions total max-pieces max-value)
  (define (make-partition partitionlist total max-pieces max-value)
  (cond 
    ((< total 0) '())
    ((= total 0) (cons partitionlist '()))
    ((= max-pieces 0) '())
    ((= max-value 0) '())
    (else (append (make-partition (append partitionlist (cons max-value '())) (- total max-value) (- max-pieces 1) max-value) 
                  (make-partition partitionlist total max-pieces (- max-value 1)) 
                  ) 
    ) 
    ) 
    ) 
    (make-partition '() total max-pieces max-value)) 
