...
class Frame:
    ...
    def make_call_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError('bad number of parameters and arguments')
        else:
...
class PrimitiveProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        python_arguments = []
        while args != nil:
            python_arguments.append(args.first)
            args = args.second
        if self.use_env:
            python_arguments.append(env)
        try:
            return self.fn(*python_arguments), None
        except TypeError:
            raise SchemeError('Scheme Error')
...
def do_define_form(vals, env):
    ...
    if scheme_symbolp(target):
        ...
        if scheme_symbolp(target.first):
            env.define(target.first, do_lambda_form(Pair(target.second, vals.second), env)[0])
            return target[0], None
        else:
            raise SchemeError("bad variable type")
    else:
...
def do_let_form(vals, env):
    ...
    for elem in bindings:
        names = Pair(elem[0], names)
        values = Pair(scheme_eval(elem[1], env), values)
...
def do_and_form(vals, env):
    ...
    elif not scheme_eval(vals.first, env):
